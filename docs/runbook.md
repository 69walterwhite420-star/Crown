...
- **Локальный стенд M0 (канистра-наблюдатель):**
  ...
  **`schnorr_key_name = opt "key_1"` ОБЯЗАТЕЛЬНО** на нынешней локальной реплике: без него дефолт
  ...
  Дельты споров (`DISPUTE_WON/LOST`) ТЕПЕРЬ ТОЖЕ переживают reinstall: их исход заякорен в цепи
  spl-memo арбитра при `resolve_dispute`, и эскроу-бэкфилл поднимает их заново (yellow-paper §18.5-8a).
  Реконструируются лишь споры, зарезолвленные уже с этой версией канистры (у старых `resolve_dispute`
  memo нет). Раньше дельты споров при reinstall терялись безвозвратно — это закрыто.
