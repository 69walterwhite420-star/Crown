...
- **Публичный экспорт** — `GET /api/v1/export/channel/[handle]` (канал + аттестация + версии конфига +
  журнал + лидерборд) и `GET /api/v1/export/anchor` (полный журнал, конфиги, ПЕР-ЗАПИСНЫЕ хэши лога
  модерации — контент инцидентов приватен и наружу не уходит, §4.6).
...
- **Пруф-якорь** — `server/anchor.ts` (+ тик в `indexer-service`): дайджесты журнала/конфигов/лога
  модерации периодически публикуются memo-tx (env `ANCHOR_SIGNER_KEYPAIR`, платит только свой газ —
