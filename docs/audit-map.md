...
| H1 | payout диктуется сервером | закрыто — ed25519-аттестация владельца (без нового контракта): клиент донора и сервер на ОБЕИХ денежных ветках проверяют подпись (обычный донат — `ingest`; эскроу-задание — `create`, ESC-20); канал без подписи → донаты приостановлены (fail-closed). Остаточное доверие: привязка handle→owner — платформенная (задокументировано в `trust-layers.md`) | 0019 |
...
`ChannelConfig`/`ConfigPatch`, enforcement в `handlers.create`/`raiseDispute`, настройка в `/space` (Personal Space → Mini-games / Dispute params).
...
| ESC-20 | LOW→MED | H1-асимметрия: серверная аттестация payout (§3.3) проверялась только на обычном донате (`ingest`), а на эскроу-ветке — лишь клиентом (`chain-provider.assertPayoutAttested`). Клиент обходится руками собранным запросом → канал с НЕподтверждённым (возможно подменённым) payout мог зеркалить эскроу-задания и капать репутацию, тогда как обычный донат ему отказывал. Прямой эксплойт ограничен (эскроу-`streamer` неизменяем ончейн, честный клиент донора гейтит фандинг), но нарушало fail-closed принцип «сервер не верит клиенту» | **закрыто** | `handlers.ts → create`: при `escrowTaskId` требует `ctx.channelPayoutAttested` (предвычислено провайдером через `verifyPayoutAttestation`, как `isChannelManager`) → иначе `PAYOUT_UNATTESTED`. Зеркало серверного гварда `ingest.ts` на второй денежной ветке. Тесты `handlers.test.ts` (H1 fail-closed: chain-эскроу к неаттестованному payout отклонён; неаттестованный payout не мешает mock/api-заданию без escrowTaskId) |
