...
8a. **Дельта канона на переключении M1 (dev-стенд; найдено сверкой M0, измерено при включении
   ...
   **✓ Закрыто (2026-07-06): дельты спора реконструируются из цепи (memo-якорь исхода).** Раньше
   ...
   пропадали безвозвратно. Теперь исход спора ЯКОРИТСЯ в цепи: когда арбитр шлёт свою тресхолд-tx
   `resolve_dispute`, он добавляет ВТОРОЙ инструкцией **spl-memo** `d1:<спорящий>:<w|l|n>` (спорящий из
   `task.dispute.by`; знак = `rep_effects`: голос «выполнено»→`l`, «не выполнено»→`w`, кворум-не-набран/
   ничья→`n`). Спорящий остаётся БЕЗ ГАЗА — на цепь шлёт резолвер, спорящий по-прежнему лишь подписывает
   в канистру; эскроу-программа НЕ меняется (memo — отдельная инструкция). Эскроу-индексатор
   (`escrow_index.rs`) читает этот memo и реконструирует ту же запись журнала с ТОЙ ЖЕ подписью, что и
   арбитр (`dispute:<pda>:DISPUTE_WON|DISPUTE_LOST`). Пока арбитр жив — он банкует первым, реконструкция
   дедупается по подписи (SEEN); после reinstall арбитр пуст, и бэкфилл поднимает дельты заново из цепи
   (resolve раньше claim: старые→новые). **Знак берётся из memo, НЕ из направления денег:** кворум-не-
   набран/ничья уводят деньги стримеру, но репутацию НЕ двигают (`n`) — наивный вывод «деньги стримеру ⇒
   спорящий проиграл» был бы неверен. Награды (бонус/штраф) — действующие governance-параметры канала на
   момент реконструкции (у арбитра — на резолве; расхождение лишь если параметры менялись И арбитр не
   банковал первым). Компромисс: споры, зарезолвленные ДО этой версии (их `resolve_dispute` без memo),
   не реконструируются (арбитра для них тоже нет). **Побочный фикс (аудит памяти 2026-07-06):** курсор
   эскроу-индексатора и карта споров арбитра оба висели на `MemoryId(6)` — коллизия виртуальной памяти
   (запись курсора затирала BTree споров). Разведено: escrow-cursor→id8, dispute-onchain→id9.
