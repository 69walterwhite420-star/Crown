...
- **Порог репутации на действия спора виден заранее** (`EscrowTaskPanel::TaskCard`, 2026-07-06):
  если единственное, что мешает зрителю открыть спор или проголосовать, — мало Reign, кнопка
  задизейблена + под карточкой честная подсказка «нужно X очков (у тебя Y)» (тон/паттерн — как у
  гейта создания задания `EscrowTaskRail::lowRep`). Раньше кнопка была активна, зритель подписывал и
  ловил сырой отказ канистры уже ПОСЛЕ подписи. Пороги: открыть — `max(minReputationToDispute, штраф)`
  из канистры (chain) или `config.minReputationToDispute` (оффчейн); голос — `minWeightToVote` (chain).
  Данные — `useStanding`+`useDisputeParams`+`useChannelConfig`; условия кнопок сведены в scenario-флаги.
...
8a. **Дельта канона на переключении M1 (dev-стенд; найдено сверкой M0, измерено при включении
   ...
   **✓ Закрыто (2026-07-06): дельты спора реконструируются из цепи (memo-якорь исхода).** Раньше
   `DISPUTE_WON`(+бонус)/`DISPUTE_LOST`(−штраф) были ЧИСТО КАНИСТРОВЫМ состоянием (кто открыл спор +
   вердикт): открытие спора — ed25519-подпись в канистру (не Solana-tx), а из цепи виден лишь резолв,
   но не получатель ±дельты и не знак. `dfx start --clean` / reinstall стирал арбитра → прошлые дельты
   пропадали безвозвратно. Теперь и исход, и авторство спора ЯКОРЯТСЯ в цепи ДВУМЯ memo арбитра (обе
   его tx тресхолд-подписаны, memo — отдельная инструкция, эскроу-программа НЕ меняется; спорящий БЕЗ
   ГАЗА — на цепь шлёт резолвер):
   • **`resolve_dispute` → `d1:<спорящий>:<дельта micro-очков>`** (ТОЧНАЯ подписанная дельта, не знак):
     `rep_effects` — голос «выполнено»→ −штраф, «не выполнено»→ +бонус, кворум-не-набран/ничья→ 0.
     `escrow_index.rs` реконструирует ту же запись журнала с ТОЙ ЖЕ подписью, что и арбитр
     (`dispute:<pda>:DISPUTE_WON|DISPUTE_LOST`, знак = знак дельты). Пока арбитр жив — банкует первым,
     реконструкция дедупается по подписи (SEEN); после reinstall арбитр пуст, бэкфилл поднимает дельты
     из цепи (resolve раньше claim: старые→новые). **Величину берём ИЗ MEMO, а не из текущих governance-
     параметров** — награды настраиваемы и меняются, а зафиксированная в цепи дельta делает реконструкцию
     точной и неизменной при поздних правках наград (детерминизм §4.4). **И не из направления денег:**
     кворум-не-набран/ничья уводят деньги стримеру при дельте 0 — «деньги стримеру ⇒ проиграл» был бы неверен.
   • **`mark_disputed` → `do1:<спорящий>:<канал>:<ed25519-подпись>`** (пруф ОТКРЫТИЯ): подпись спорящего
     над сообщением открытия (`build_open_message`) кладётся в цепь. Любой аудитор восстанавливает
     сообщение из (эскроу в аккаунтах tx, канал+спорящий из memo) и сверяет ed25519 → «кто открыл»
     доказуемо ИЗ ЦЕПИ, без доверия канистре (канистра лишь «почтальон» — подделать чужую подпись не
     может). Индексатору для реконструкции это memo не нужно — оно чисто для внешнего аудита.
   memo-декод: RPC отдаёт jsonParsed (`program:"spl-memo"`+`parsed`), с fallback на partiallyDecoded.
   Компромисс: споры, зарезолвленные ДО этой версии (их `resolve_dispute` без memo либо со старым `w|l|n`),
   не реконструируются. **Побочный фикс (аудит памяти):** курсор эскроу-индексатора и карта споров арбитра
   оба висели на `MemoryId(6)` — коллизия виртуальной памяти (запись курсора затирала BTree споров).
   Разведено: escrow-cursor→id8, dispute-onchain→id9.
   **✓ Закрыто (2026-07-06): эскроу-донаты заданий (`GameDonation`) канистра теперь индексирует.**
   ...
   в конфиге) отдельным курсором опрашивает подписи эскроу-программы, декодит `fund`/`resolve_dispute`/
   `claim_streamer`/`claim_donor` по anchor-дискриминаторам (зеркало `escrow-tx.ts`): `fund` → снимок
   {донор, стример, сумма} в stable-карту (эскроу-аккаунт закрывается при claim); `resolve_dispute` →
   читает memo арбитра, помечает эскроу оспоренным и банкует ±репутацию спорящему (см. 8a); `claim_streamer`
   → `GameDonation` донору на канал стримера (очки = полная сумма 1:1, паритет `repEffects`; у оспоренных
   подпись денег `dispute:<pda>:DONATION` — паритет арбитра); `claim_donor` (возврат) → репутации нет.
   **Дедуп с оспоренными теперь по цепи, НЕ по `arbiter::case_of`** (то было канистровое состояние, гибнет
   при reinstall — из-за чего дельты и терялись): реконструкция использует ТЕ ЖЕ подписи журнала, что и
   арбитр, и дедупается по SEEN. Идемпотентность — по подписи записи (SEEN). Привязка стример→канал:
   активация канала в журнале (actor = владелец = payout, допущение v1); канал mock-эпохи без ончейн-
   активации не резолвится → запись пропускается (та же дельта, что у чтений выше).
