...
`extends ChainDataProvider`, переопределены ТОЛЬКО `getStanding`/`getLeaderboard`: браузер
читает канон репутации напрямую из HTTP-экспорта core-канистры (`/standing`, `/leaderboard`;
CORS открыт на чтение) МИМО нашего сервера. Тиры — `resolveTier` по конфигу канала (кожа);
...
- **G3b (до mainnet)**: ончейн commit-reveal голосование (снимает §18.1-1/2/3), депозит спора,
  D_max молодых каналов, commit-reveal ≥$50 (whitepaper), уход `ESCROW_RESOLVER`. Субстрат —
...
- **КОСТЬ №2 (строится, ADR 0021)**: `canister/core/src/` — `donation.rs`/`reputation.rs`/
  `disputes.rs` (порты трастлесс-логики; паритет с TS держат golden-тесты
  `canister/core/tests/golden.rs` ↔ `testdata/golden/`, экспортёр `scripts/export-golden.ts`),
